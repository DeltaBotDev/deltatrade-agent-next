...
function initSdk(env: Env) {
  return DeltaTradeSDK.initEnv({
    chain: 'near',
    network: env.NEAR_ENV as 'mainnet' | 'testnet' || 'mainnet',
  });
}


export async function handleGetPairs(request: Request, corsHeaders: any, env: Env) {
    try {
      const url = new URL(request.url);
      const type = url.searchParams.get('type') || 'dca' as any ;
      
      const sdk = initSdk(env);
  ...
  export async function handleCreateDCA(request: Request, corsHeaders: any, env: Env) {
    try {
